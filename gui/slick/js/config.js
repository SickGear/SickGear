function toggle$(el, cond){
	let hide = '.hide', show = '.show', t;
	const ifId = '-if-' + $(el).attr('id');
...
$(document).ready(function () {
	...
	elOnOff.on('click', function(_e) {
		const strList = $.trim(elInput.val());
	...
	$('#configForm').ajaxForm({
		beforeSubmit: function () {
			$('.config_submitter').each(function () {
				...
				$(this).after(`<span><img src="${sbRoot}/images/loading16${themeSpinner}.gif" alt=""> Saving...</span>`);
				$(this).hide();
			});
	...
	const addQR = function(){
		if (!!$('a[rel=qr]').length){
			if ('function' === typeof($.fn.fancybox)){
				updateFB();
			} else {
				const fancy = sbRoot + '/js/fancybox/jquery.fancybox.min';
	...
	$('#showlist_tagview').on('change', function() {
		const selected = '#showlist_tagview_', target = $(selected + 'custom_config'), thisVal = $(this).val();
		target.removeClass('hidden');
		if ('custom' !== thisVal)
			target.addClass('hidden');
		$(`${selected}standard, ${selected}anime, ${selected}custom`).removeClass('hidden').addClass('hidden');
		$(`${selected}${thisVal}`).removeClass('hidden');
	});
});
