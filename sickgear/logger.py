...
class SBRotatingLogHandler(object):
    ...
    def init_logging(self, console_logging=False):
        ...
        self.log_file_path = os.path.join(sickgear.LOG_DIR, self.log_file)
        ...
        if self.console_logging:
            ...
            if None is not h_console.stream:
                h_console.setLevel((logging.INFO, logging.DEBUG)[sickgear.DEBUG])
        ...
        h_file.setLevel(reverseNames[sickgear.FILE_LOGGING_PRESET])
        ...
        log_level = (logging.WARNING, logging.DEBUG)[sickgear.DEBUG]
        for logger_name in [x for x in self.log_types if 'sickgear' != x]:
            logging.getLogger(logger_name).setLevel(log_level)
        logging.getLogger('sickgear').setLevel(DB)
    ...
    def log_error_and_exit(self, error_msg):
        ...
        if not self.console_logging:
            sys.exit(error_msg.encode(sickgear.SYS_ENCODING, 'xmlcharrefreplace'))
        else:
            sys.exit(1)
...
class TimedCompressedRotatingFileHandler(TimedRotatingFileHandler):
    ...
    def doRollover(self):
        ...
        example:
        logger.TimedCompressedRotatingFileHandler(sickgear.logger.sb_log_instance.log_file_path, when='M', interval=2,
                                                  logger=sickgear.logger.sb_log_instance).doRollover()
        """
...
def load_log(load_msg, to_log, log_level=MESSAGE):
    ...
    if MESSAGE == log_level:
        sickgear.classes.loading_msg.set_msg_progress(load_msg, to_log)
...
def log_set_level():
    if sb_log_instance.h_file:
        sb_log_instance.h_file.setLevel(reverseNames[sickgear.FILE_LOGGING_PRESET])


def current_log_file():
    return os.path.join(sickgear.LOG_DIR, sb_log_instance.log_file)
