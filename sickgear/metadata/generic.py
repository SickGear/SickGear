...
class GenericMetadata(object):
    ...
    def get_show_year(show_obj, show_info, year_only=True):
        # type: (sickgear.tv.TVShow, Dict, bool) -> Optional[AnyStr]
        if None is not getattr(show_info, 'firstaired', None):
            try:
                ...
                pass
        if isinstance(show_obj, sickgear.tv.TVShow):
            if year_only and show_obj.startyear:
                return f'{show_obj.startyear}'
            ...
            try:
                ...
                first_ep_obj = None
            if isinstance(first_ep_obj, sickgear.tv.TVEpisode) \
                    and isinstance(first_ep_obj.airdate, datetime.date) and 1900 < first_ep_obj.airdate.year:
                return f'{(first_ep_obj.airdate.year, first_ep_obj.airdate)[not year_only]}'
    ...
    def get_episode_thumb_path(self, ep_obj):
        ...
        if os.path.isfile(ep_obj.location):
            ...
            return f'{tbn_filename}-thumb.jpg'
    ...
    def get_season_banner_path(self, show_obj, season):
        ...
        if 0 == season:
            ...
            season_banner_filename = f'season{str(season).zfill(2)}'
    ...
    def _valid_show(fetched_show_info, show_obj):
        # type: (Dict, sickgear.tv.TVShow) -> bool
        """
    ...
    def create_show_metadata(self, show_obj, force=False):
        # type: (sickgear.tv.TVShow, bool) -> bool
        result = False
        if self.show_metadata and show_obj and (not self._has_show_metadata(show_obj) or force):
            logger.debug(f'Metadata provider {self.name} creating show metadata for {show_obj.unique_name}')
            try:
    ...
    def create_episode_metadata(self, ep_obj, force=False):
        ...
        if self.episode_metadata and ep_obj and (not self.has_episode_metadata(ep_obj) or force):
            logger.debug(f'Metadata provider {self.name} creating episode metadata for {ep_obj.pretty_name()}')
            try:
    ...
    def update_show_indexer_metadata(self, show_obj):
        ...
        if self.show_metadata and show_obj and self._has_show_metadata(show_obj):
            ...
            if None is not tvid:
                tvid.text = f'{show_tvid}'
            else:
                etree.SubElement(root, 'indexer').text = f'{show_tvid}'

            show_prodid = str(show_obj.prodid)
            if None is not prodid:
                prodid.text = f'{show_prodid}'
            else:
                etree.SubElement(root, 'id').text = f'{show_prodid}'
        ...
        for cur_ep_obj in ep_obj_list:
            if TVINFO_TVDB == cur_ep_obj.show_obj.tvid:
                ...
                try:
                    ...
                    t = sickgear.TVInfoAPI(TVINFO_TVDB).setup(**tvinfo_config)

                    ep_info = t.get_show(cur_ep_obj.show_obj.prodid, language=show_lang)[
    ...
    def save_fanart(self, show_obj, which=None):
        # type: (sickgear.tv.TVShow, Optional[AnyStr]) -> bool
        """
            ...
            if 'poster_thumb' == image_type:
                ...
                if None is not getattr(show_infos[tv_src], image_type, None):
                    ...
                    if image_type in ('poster', 'banner'):
                        if None is not getattr(show_infos[tv_src], f'{image_type}_thumb', None):
                            thumb_url = show_infos[tv_src][f'{image_type}_thumb']
                        else:
                            thumb_url = image_url
                    else:
                        thumb_url = None
    ...
    def _retrieve_show_image(self,
        ...
        if image_urls:
            if return_links:
                ...
                img_data = None
                image_cache = sickgear.image_cache.ImageCache()
    ...
    def _season_image_dict(self, show_obj, season, image_type):
        # type: (sickgear.tv.TVShow, int, AnyStr) -> Dict[int, Dict[int, AnyStr]]
        """
    ...
    def retrieve_show_metadata(self, folder):
        ...
        from sickgear.indexers.indexer_config import TVINFO_TVDB
    ...
    def _fanart_urls_from_show(self, show_obj, image_type='banner', lang='en', thumb=False):
        # type: (sickgear.tv.TVShow, AnyStr, AnyStr, bool) -> Optional[List[int, int, AnyStr]]
        try:
    ...
    def retrieve_show_image(self, image_type, show_obj, which=None, return_links=False, show_infos=None):
        # type: (AnyStr, sickgear.tv.TVShow, bool, bool, ShowInfosDict) -> Optional[bytes]
