...
from hachoir_py2.field import (Field, FieldSet, createOrphanField,
                               NullBits, Bit, Bits, Enum, Fragment, MissingField, ParserError,
                               UInt8, UInt16, UInt24, UInt32, UInt64,
                               RawBytes, String, PascalString32, NullBytes)
from hachoir_py2.stream import FragmentedStream, InputStreamError
from hachoir_py2.core.endian import LITTLE_ENDIAN, BIG_ENDIAN
from hachoir_py2.core.tools import humanDurationNanosec
...
class OggPage(FieldSet):
    ...
    def createFields(self):
        ...
        if self['capture_pattern'].value != self.MAGIC:
            self.warning(
                'Invalid signature. An Ogg page must start with "%s".' % self.MAGIC)
