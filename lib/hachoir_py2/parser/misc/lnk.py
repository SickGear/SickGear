...
from hachoir_py2.parser.common.win32 import GUID
from hachoir_py2.parser.common.msdos import MSDOSFileAttr16, MSDOSFileAttr32
from hachoir_py2.core.text_handler import filesizeHandler
...
class FileLocationInfo(FieldSet):
    def createFields(self):
        ...
        yield UInt32(self, "local_pathname_offset",
                     "Offset to local base pathname; only meaningful if on_local_volume = 1")
        yield UInt32(self, "remote_info_offset",
        ...
        if has_unicode_paths:
            yield UInt32(self, "local_pathname_unicode_offset",
        ...
        if self["on_local_volume"].value:
            ...
            if has_unicode_paths:
                ...
                if padding:
                    yield padding
                yield CString(self, "local_base_pathname_unicode", "Local Base Pathname in Unicode",
                              charset="UTF-16-LE")
        ...
        if has_unicode_paths:
            ...
            if padding:
                yield padding
            yield CString(self, "final_pathname_unicode", "Final component of the pathname in Unicode",
                          charset="UTF-16-LE")
...
class ExtraInfo(FieldSet):
    ...
    def createFields(self):
        ...
        yield Enum(
            textHandler(UInt32(self, "signature", "Signature determining the function of this structure"), hexadecimal),
            self.INFO_TYPE)
        ...
        if self["signature"].value == 0xA0000003:
            ...
            yield String(self, "font_name_unicode", 64, "Font Name (Unicode format)", charset="UTF-16-LE",
                         truncate="\0")
            ...
            yield Enum(UInt32(self, "quick_edit", "Console uses quick-edit feature (using mouse to cut & paste)?"),
                       self.BOOL_ENUM)
            ...
            yield Enum(UInt32(self, "history_no_dup", "Automatically eliminate duplicate lines in the history buffer?"),
                       self.BOOL_ENUM)
