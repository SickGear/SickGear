...
from hachoir_py2.core.text_handler import displayHandler
...
class BlockAddress(FieldSet):
    ...
    def createFields(self):
        yield displayHandler(Bits(self, "offset", 27, description="Offset into file divided by 32"),
                             lambda x: hex(x * 32).strip('L'))
        yield displayHandler(Bits(self, "size", 5, description="Power-of-2 size of the block"),
...
class BudHeader(FieldSet):
    ...
    def createFields(self):
        ...
        yield UInt32(self, "allocator_offset_backup",
                     description="Finder will refuse to read the file if this does not match the first copy")
        for i in xrange(4):
            yield BlockAddress(self, "block_address_copy[]",
...
class BudAllocator(FieldSet):
    def createFields(self):
        ...
        if padding:
            yield NullBytes(self, "padding", padding * 4,
                            description="padding to make the number of blocks a multiple of 256")
