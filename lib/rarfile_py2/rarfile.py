...
from _23 import native_timestamp
...
class RarFile(object):
    ...
    def __init__(self, file, mode="r", charset=None, info_callback=None,
        ...
        if is_filelike(file):
            ...
            if _have_pathlib and isinstance(file, Path):
                file = str(file)
            self.filename = file
        self._rarfile = file
    ...
    def _extract_one(self, info, path, pwd, set_attrs):
        ...
        if path is None:
            ...
            if not isinstance(path, (string_types, bytes)):
                return None
    ...
    def _make_symlink(self, info, dstfn, pwd, set_attrs):
        if info.host_os == RAR_OS_UNIX:
            ...
            if redir_type == RAR5_XREDIR_WINDOWS_JUNCTION:
                warnings.warn("Windows junction not supported - %s" % info.filename, UnsupportedWarning)
                return None
        else:
            warnings.warn("Unsupported link type - %s" % info.filename, UnsupportedWarning)
            return None

        os.symlink(link_name, dstfn)
        return dstfn
    ...
    def _set_attrs(self, info, dstfn):
        ...
        if info.mtime:
            ...
            os.utime(dstfn, (atime_ns, mtime_ns))
        ...
        while pos < len(hdata):
            ...
            if stype == RAR_BLOCK_OLD_COMMENT and pos + S_COMMENT_HDR.size <= pos_next:
                ...
                cmt = rar3_decompress(ver, meth, data, declen, sflags,
                                      crc, self._password)
                if not self._crc_check or (rar_crc32(cmt) & 0xFFFF == crc):
...
class RAR5Parser(CommonParser):
    ...
    def _parse_block_header(self, fd):
        ...
        if len(start_bytes) < preload:
            self._set_error("Unexpected EOF when reading header")
            return None
        while bytearray(start_bytes)[-1] & 0x80:
            b = fd.read(1)
            if not b:
                self._set_error("Unexpected EOF when reading header")
                return None
...
class PipeReader(RarExtFile):
    ...
    def _open_extfile(self, parser, inf):
        super(PipeReader, self)._open_extfile(parser, inf)
...
class DirectReader(RarExtFile):
    ...
    def __init__(self, parser, inf):
        super(DirectReader, self).__init__()
        self._open_extfile(parser, inf)

    def _open_extfile(self, parser, inf):
        super(DirectReader, self)._open_extfile(parser, inf)
...
class nsdatetime(datetime):
    ...
    def __new__(cls, year, month, day, hour=0, minute=0, second=0,
        ...
        if mod == 0:
            return datetime(year, month, day, hour, minute, second, usec, tzinfo)
        self = super(nsdatetime, cls).__new__(cls, year, month, day, hour, minute, second, usec, tzinfo)
    ...
    def isoformat(self, sep="T", timespec="auto"):
        ...
        """
        if timespec == "auto":
            pre, post = super(nsdatetime, self).isoformat(sep, "microseconds").split(".", 1)
            return "{}.{:09d}{}".format(pre, self.nanosecond, post[6:])
        return super(nsdatetime, self).isoformat(sep, timespec)

    def astimezone(self, tz=None):
        """Convert to new timezone.
        """
        tmp = super(nsdatetime, self).astimezone(tz)
    ...
    def __hash__(self):
        return super(nsdatetime, self).__hash__()

    def __eq__(self, other):
        return super(nsdatetime, self).__eq__(other)

    def __gt__(self, other):
        return super(nsdatetime, self).__gt__(other) or (super(nsdatetime, self).__eq__(other))
...
def custom_popen(cmd):
    ...
    # run command
    try:
        p = Popen(cmd, bufsize=0, stdout=PIPE, stderr=STDOUT, stdin=open(os.devnull, 'w'),
