...
from tornado_py2.concurrent import Future, future_set_exc_info
from tornado_py2.escape import utf8
from tornado_py2 import gen
import tornado_py2.ioloop
from tornado_py2.log import app_log
from tornado_py2.netutil import Resolver
from tornado_py2.stack_context import NullContext, wrap
...
class TornadoReactor(PosixReactorBase):
    ...
    """
    def __init__(self):
        self._io_loop = tornado_py2.ioloop.IOLoop.current()
...
class _FD(object):
    ...
    def doRead(self):
        if not self.lost:
            self.handler(self.fileobj, tornado_py2.ioloop.IOLoop.READ)

    def doWrite(self):
        if not self.lost:
            self.handler(self.fileobj, tornado_py2.ioloop.IOLoop.WRITE)

    def connectionLost(self, reason):
        if not self.lost:
            self.handler(self.fileobj, tornado_py2.ioloop.IOLoop.ERROR)
            self.lost = True
...
class TwistedIOLoop(tornado_py2.ioloop.IOLoop):
...
class TwistedResolver(Resolver):
    ...
    def initialize(self):
        ...
        self.reactor = tornado_py2.platform.twisted.TornadoReactor()
